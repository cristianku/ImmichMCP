...
void ConfigureMcpCapabilities(McpServerOptions options, IConfiguration configuration)
{
    options.ServerInstructions = """
        ImmichMCP searches and manages the user's private Immich library. For photo requests, first use the appropriate search, people, or asset tool to identify matching asset IDs and sort the final matches newest first. Inspect no more than three candidate previews unless the user explicitly asks for a broader comparison. When the user asks to see, show, display, browse, inspect, or visualize photos, call immich_assets_download_thumbnail for each final asset. With DOWNLOAD_MODE=base64, its text result contains model-readable Base64 in result.data and its MCP result also contains an inline image block. For the final selected photo, decode result.data into a local file using result.mime_type and embed the resulting sandbox file in the final answer. The technical tool preview alone is not the final displayed photo. Immediately below every embedded photo, add a concise caption from the thumbnail metadata in the form "📍 location — 📅 captured date/time". Omit whichever part is unavailable and never infer a location or date. Never use immich_assets_download_original for previews. Never answer a request to see photos with filenames or metadata alone, and never claim a photo is shown unless it is actually embedded in the final answer.
        """;
