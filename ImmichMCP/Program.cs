...
void ConfigureMcpCapabilities(McpServerOptions options, IConfiguration configuration)
{
    options.ServerInstructions = """
        ImmichMCP searches and manages the user's private Immich library. For photo requests, first use the appropriate search, people, or asset tool to identify matching asset IDs. When the user asks to see, show, browse, inspect, or find photos, always finish by calling immich_assets_show with the final matching asset IDs, newest first. immich_assets_show is the only tool that displays photos visibly to the user. Never use immich_assets_download_original or immich_assets_download_thumbnail to display a photo, and never claim that those tools displayed an image. ChatGPT has no filesystem provided by this MCP: never save or claim to save files under /mnt/data, never create or link to sandbox: paths, and never ask the user to download an original as a substitute for showing a photo. Do not answer a request to see photos with filenames or metadata alone. The gallery is read-only and displays private thumbnail previews.
        """;
